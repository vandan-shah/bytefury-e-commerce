...
<body>
    <section class="text-gray-600 body-font">
        <div class="container px-5 py-24 mt-28 mx-auto items-center lg:w-2/6 md:w-1/2">
            <form method="POST" action="customerlogin" class="bg-gray-100 rounded-lg p-8 flex flex-col md:ml-auto w-full mt-10 md:mt-0">
                @csrf
                <h2 class="text-gray-900 text-lg font-medium title-font mb-5">Login</h2>
                <div class="relative mb-4">
                    <label for="email" class="leading-7 text-sm text-gray-600">Email</label>
                    <input type="text" id="email" name="email" class="w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" @error('email') is-invalid @enderror value="{{ old('email') }}" required  />
                ...
                <a href="{{route('customerregistration')}}" class="text-xs text-gray-500 mt-7 font-bold cursor-pointer place-self-center">New to NeoCyber? CREATE ACCOUNT</a>
            </form>
        </div>
    </section>
